...
\textbf{Date:} & June 2026 \\[8pt]
\textbf{Platform:} & Rocky Linux 9.8 + VirtualBox
\end{tabular}

\vfill
{\small Version: v2.1 (Rocky Linux 9.8)}
\end{center}
\end{titlepage}
...
实验2（VPN）、实验6（Samba）、实验8（NAT）需要两台虚拟机协同实验。使用 VirtualBox 克隆功能可快速生成第二台虚拟机，无需重新安装系统。（实验4采用单机三网卡自闭环方案，详见子项目4。）
...
\begin{enumerate}[label=\arabic*.,leftmargin=*]
  ...
  \item \textbf{修复原有 repo 中的 \$contentdir 变量}

  Rocky 默认 repo 可能使用 \texttt{\$contentdir} 变量，需先替换为 \texttt{rocky}：
...
sed -i 's|$contentdir|rocky|g' /etc/yum.repos.d/rocky*.repo
  ...
  \item \textbf{备份原有配置}（备份目录带日期戳，支持重复执行）
...
客户端需要从服务端获取3个证书文件，然后编写 client.conf 并启动连接。最简便的方式是直接运行客户端脚本（不带参数时会自动扫描 Host-Only 网络发现服务端 192.168.56.101/100/1）：

\begin{cmdbox}
\begin{lstlisting}[language=bash]
# 自动发现服务端
sudo bash labs/lab2_VPN/client_setup.sh
# 或显式指定服务端 IP
sudo bash labs/lab2_VPN/client_setup.sh 192.168.56.101
\end{lstlisting}
\end{cmdbox}

以下手动步骤等价于该脚本所做的工作（脚本自动用 \texttt{\$\{SUDO\_USER:-\$USER\}} 检测当前用户名）：
...
使用 Postfix 搭建 SMTP 邮件发送客户端，通过 QQ 邮箱或 163 邮箱的 SMTP 服务器中继实现邮件发送功能。

\subsection{背景说明}

\textbf{本实验并非搭建完整公网邮件服务器。} 完整邮件系统需 Postfix + Dovecot + 自有域名 + MX DNS 记录。本实验中的 Postfix 仅作为 SMTP 中继客户端，借用 QQ 或 163 邮箱的 SMTP 服务器代理发送邮件。脚本会根据邮箱后缀（\texttt{@qq.com} / \texttt{@163.com}）自动选择对应的 SMTP 服务器。

\textbf{工作流程}：Postfix 接收本地邮件 → SASL 认证登录 SMTP 服务器（需邮箱地址 + 授权码）→ 邮件服务商验证身份后以该邮箱名义将邮件投递到最终收件人。

Postfix 作为工业级 MTA，通过纯文本配置文件即可操控完整 SMTP 协议行为：\texttt{relayhost} 指定中继目标、\texttt{smtp\_use\_tls} 控制传输加密、\texttt{smtp\_sasl\_auth\_enable} 启用身份认证。

\subsubsection{获取邮箱授权码}

脚本运行时需输入发件邮箱地址和对应授权码。目前支持两种邮箱：

\begin{center}
\begin{tabular}{lcl}
\toprule
提供商 & SMTP 服务器:端口 & 授权码获取方式 \\
\midrule
QQ 邮箱 & \texttt{smtp.qq.com:587} & 登录 mail.qq.com → 设置 → 账户 → POP3/SMTP 服务 → 开启 \\
163 邮箱 & \texttt{smtp.163.com:465} & 登录 mail.163.com → 设置 → POP3/SMTP/IMAP → 开启 \\
\bottomrule
\end{tabular}
\end{center}

两种邮箱授权码获取方式类似：开启 SMTP 服务后，按提示发送短信验证，即可获得授权码。

\textbf{配置文件 → 程序解析 → 协议行为变化}这一范式贯穿全部 8 个实验，是本课程的核心教学模式。
...
yum install -y postfix s-nail cyrus-sasl-plain
  ...
  在 \texttt{/etc/postfix/main.cf} 末尾添加（\texttt{\$SMTP\_HOST} 和 \texttt{\$SMTP\_PORT} 由脚本自动检测，标记行用于支持重复执行时不产生重复配置）：
...
EOF
  ...
  \item \textbf{创建认证文件和发件人地址映射}

  \begin{cmdbox}
  \begin{lstlisting}[language=bash]
echo "[$SMTP_HOST]:$SMTP_PORT ${SENDER_EMAIL}:${AUTH_CODE}" > /etc/postfix/sasl_passwd
...
postmap /etc/postfix/generic
  ...
  \texttt{smtp\_generic\_maps} 强制将所有本地发件人地址（如 \texttt{root}）改写为授权邮箱，满足 163 服务器 \texttt{Mail from must equal authorized user} 的要求。
  \item \textbf{启动并测试}
...
\begin{itemize}
  \item postfix 服务正在运行
  \item main.cf 中 relayhost 已配置
  \item /etc/postfix/sasl\_passwd 和 .db 文件存在
  \item postfix check 语法检查通过
  \item 发送测试邮件成功（默认发至 nucleocore2026@126.com，可自定义）
\end{itemize}
...
本实验采用 \textbf{单机三网卡自闭环} 方案：给 server VM 新增第三张内部网络网卡（NIC3），dnsmasq 绑定内网 IP，
同一台 VM 通过 \texttt{dhclient} 向自己的 dnsmasq 获取 DHCP 地址，实现 DHCP + DNS 全流程验证，\textbf{无需第二台虚拟机}。

\subsection{网卡架构}

\begin{table}[h]
\centering
\begin{tabular}{p{2.4cm}p{2.8cm}p{2.8cm}p{4.6cm}}
\toprule
网卡 & 连接方式 & IP 来源 & 用途 \\
\midrule
NIC1 (enp0s3) & NAT & VirtualBox DHCP & 访问外网（yum install、DNS 转发）\\
NIC2 (enp0s8) & Host-Only & 静态 192.168.56.x & Windows SSH 连接 \\
NIC3 (enp0s9，新增) & \textbf{内部网络} & 静态 10.100.0.1/24 & dnsmasq DHCP + DNS 验证 \\
\bottomrule
\end{tabular}
\end{table}

\begin{warnbox}
  NIC3 为\textbf{内部网络（Internal Network）}模式，\textbf{不是} Host-Only。
  内部网络与 NIC1/NIC2 完全隔离，不存在 VirtualBox 内置 DHCP 冲突。
\end{warnbox}

\subsection{VirtualBox 操作}

\begin{enumerate}[label=\arabic*.,leftmargin=*]
  \item 关闭 VM（关机，不是挂起）
  \item VirtualBox → 设置 → 网络 → 网卡 3 → 启用网络连接
  \item 连接方式选择：\textbf{内部网络 (Internal Network)}
  \item 名称填写：\texttt{intnet-dhcp}（可自定义）
  \item 启动 VM
\end{enumerate}
...
yum install -y dnsmasq
  ...
  \item \textbf{配置 NIC3 内部网络网卡}

  setup.sh 自动检测未配置 IP 的网卡，按顺序尝试 \texttt{enp0s9、enp0s10、enp0s16、ens9、ens10}。
  通过 \texttt{nmcli} 创建连接 \texttt{dhcp-intnet}，分配静态 IP \texttt{10.100.0.1/24}：

  \begin{cmdbox}
  \small
  \begin{lstlisting}[language=bash]
# enp0s9 为脚本自动探测到的 NIC3（也可能是 enp0s10/ens9 等，脚本内用变量 $NIC3）
nmcli connection add type ethernet ifname enp0s9 con-name dhcp-intnet \
    ip4 10.100.0.1/24 autoconnect yes
  \end{lstlisting}
  \end{cmdbox}
...
bind-interfaces
domain=tecmint.lan
server=223.5.5.5        # 阿里 DNS（外网转发）
server=223.6.6.6        # 阿里 DNS（外网转发）
address=/tecmint.lan/10.100.0.1
cache-size=150

# DHCP 配置
dhcp-range=10.100.0.50,10.100.0.150,12h
dhcp-option=3,10.100.0.1
dhcp-option=6,10.100.0.1
dhcp-leasefile=/var/lib/misc/dnsmasq.leases
  \end{lstlisting}
  \end{cmdbox}

  \begin{infobox}
    \texttt{listen-address} 只绑定内网 IP \texttt{10.100.0.1}，配合 \texttt{bind-interfaces}
    确保 dnsmasq 仅监听 NIC3，不干扰 NIC1/NIC2 的正常网络通信。
    DHCP 地址池 \texttt{10.100.0.50--150} 与 Host-Only 网段（192.168.56.x）完全隔离。
  \end{infobox}
...
# NIC3 通过 dhclient 向自己发起 DHCP 请求
dhclient enp0s9

# 检查是否获取到 IP（应落在 10.100.0.50--150）
ip addr show enp0s9 | grep 10.100.0
  \end{lstlisting}
  \end{cmdbox}
  ...
  \begin{cmdbox}
  \begin{lstlisting}[language=bash]
nslookup tecmint.lan 10.100.0.1       # 本地域名解析
nslookup www.baidu.com 10.100.0.1     # 外网 DNS 转发
  \end{lstlisting}
  \end{cmdbox}
...
\texttt{dnsmasq} 是一个轻量级网络服务程序，一个二进制、一套配置文件就同时提供 DNS 和 DHCP 两项功能。
理解它的 DNS 解析流程对掌握本实验至关重要。

\subsubsection{内网域名解析（tecmint.lan）}

用户执行 \texttt{ping tecmint.lan}，系统通过 \texttt{/etc/resolv.conf} 指向的 DNS 服务器 \texttt{10.100.0.1}
发起查询。dnsmasq 收到请求后的查找顺序：

\begin{enumerate}[label=(\arabic*),leftmargin=*]
  \item 查本地缓存 —— 之前访问过则直接返回
  \item 查 \texttt{/etc/hosts} —— 有则返回
  \item 匹配 \texttt{dnsmasq.conf} 中的 \texttt{address=/tecmint.lan/10.100.0.1} —— \textbf{命中！返回 10.100.0.1}
\end{enumerate}

整个过程在 enp0s9（内部网络）内完成，无需访问外网。

\subsubsection{外网域名解析（www.baidu.com）}

用户 \texttt{nslookup www.baidu.com 10.100.0.1} 的完整流程：

\begin{enumerate}[label=(\arabic*),leftmargin=*]
  \item DNS 请求通过 enp0s9 到达 dnsmasq（监听 \texttt{10.100.0.1}）
  \item dnsmasq 查本地缓存 → 未命中 → 查 \texttt{/etc/hosts} → 未命中
  \item 匹配 \texttt{dnsmasq.conf} 规则：
  \begin{itemize}
    \item \texttt{address=/tecmint.lan/10.100.0.1} —— 不匹配 \texttt{www.baidu.com}，跳过
    \item \texttt{server=223.5.5.5} —— \textbf{命中！转发给上游阿里 DNS}
  \end{itemize}
  \item Linux 内核路由决策：\texttt{223.5.5.5} 不在任何本地网段，走\textbf{默认路由}
  \item 出口网卡为 enp0s8 (NAT)，VirtualBox NAT 引擎执行 SNAT 后发往互联网
  \item 阿里 DNS 返回 \texttt{www.baidu.com} 的 IP 地址，原路返回 → enp0s8 → dnsmasq → 客户端
\end{enumerate}

\begin{infobox}
  DNS 解析完成后，后续的实际通信（如 \texttt{ping} 的 ICMP 包、\texttt{curl} 的 HTTP 请求）
  直接走内核路由从 enp0s8 (NAT) 出去，\textbf{与 dnsmasq 无关}。dnsmasq 只负责域名→IP 的翻译工作。
\end{infobox}

\subsubsection{三网卡分工总结}

\begin{table}[h]
\centering
\begin{tabular}{p{2.2cm}p{2.5cm}p{9cm}}
\toprule
网卡 & 角色 & 在本实验中的分工 \\
\midrule
enp0s9 (内部网络) & DNS/DHCP 入口 & 接收客户端 DNS 查询 + DHCP 请求 \\
enp0s8 (NAT)       & 外网出口   & DNS 向上游转发（访问 223.5.5.5）+ 用户实际外网流量 \\
enp0s3 (Host-Only) & SSH 管理   & Windows 远程登录，不参与实验流程 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{infobox}
  \texttt{setup.sh} 自动完成 NIC3 发现、静态 IP 配置、dnsmasq 安装配置、dhclient 验证全流程。
\end{infobox}
...
\begin{itemize}
  ...
  \item 内部网络网卡（NIC3）已配置 IP 10.100.0.1
  \item DHCP 租约文件 /var/lib/misc/dnsmasq.leases 有记录（NIC3 成功获取 IP）
...
zone "test-web2.com" IN { type master; file "web2.com.zone"; };
  ...
  创建区域文件 \texttt{/var/named/web1.com.zone}（web2 同理，仅域名替换为 test-web2），
  将 \texttt{www} 的 A 记录指向服务器 IP：
...
\subsection{从 Windows 宿主访问}

VM 的 Host-Only 网卡与 Windows 宿主互通（同一网段 192.168.56.0/24），
Apache 监听 \texttt{*:80}，\texttt{firewalld} 已放行 http 服务，可直接从 Windows 浏览器访问。

\subsubsection{PowerShell / CMD 测试}
...
\subsubsection{浏览器访问（需改 hosts）}

编辑 \texttt{C:\textbackslash Windows\textbackslash System32\textbackslash drivers\textbackslash etc\textbackslash hosts}（管理员权限），添加：
...
保存后在浏览器直接访问 \texttt{http://www.test-web1.com} 和 \texttt{http://www.test-web2.com}。
浏览器请求 HTTP 时会自动写 \texttt{Host} 头，Apache 根据所在 \texttt{<VirtualHost>} 的 \texttt{ServerName}
返回对应网页。

\begin{infobox}
  Apache 虚拟主机利用 HTTP 请求头中的 \texttt{Host} 字段区分来访域名，
  \textbf{即使两个域名解析到同一个 IP 也能返回不同网页}，这是 lab5 的核心。
\end{infobox}
...
setfacl -m g:test:rwx    /data/share/develop_test
  ...
  \item \textbf{SELinux 放行 Samba 访问}

  Rocky Linux 9 默认启用 SELinux 强制模式，Samba 仅允许访问系统标记为 \texttt{samba\_share\_t} 的路径。
  自定义目录 \texttt{/data/share} 需要手动添加上下文标记：
...
restorecon -Rv /data/share
  ...
  \begin{warnbox}
    \textbf{未配置此行将导致 Windows 无法访问共享目录}。症状：文件资源管理器提示"网络错误"、
    凭据正确但拒绝访问。常见于 Rocky Linux 9 / RHEL 9 系列等 SELinux 默认 enforcing 的系统。
  \end{warnbox}
...
\texttt{setup.sh} 已配置 \texttt{firewall-cmd --add-service=samba} 放行 139/445 端口，
...
\begin{cmdbox}
\begin{lstlisting}[language=bash,numbers=none]
\\192.168.56.101\develop
\end{lstlisting}
\end{cmdbox}

在弹出的认证窗口中输入用户名 \texttt{develop} 和密码 \texttt{123456}，即可浏览 \texttt{/data/share/develop} 目录。

不同账号登录结果不同：

\begin{itemize}
  \item \texttt{develop} 可访问 \texttt{\textbackslash\textbackslash 192.168.56.101\textbackslash develop}，不可访问 \texttt{\textbackslash\textbackslash ...\textbackslash productdesign}
  \item \texttt{productdesign} 可访问 \texttt{\textbackslash\textbackslash ...\textbackslash productdesign}，不可访问 \texttt{\textbackslash\textbackslash ...\textbackslash develop}
  \item \texttt{system}（管理员）可访问全部共享
\end{itemize}

\subsubsection{命令行映射网络驱动器}
...
服务器需要至少两张网卡：一张外网（NAT，连互联网）、一张内网（Host-Only，连客户端）。脚本自动识别：有默认路由的网卡作为外网(WAN)，另一张作为内网(LAN)，无需手动指定 eth0/eth1。
...
systemctl enable iptables --now
  ...
  \begin{cmdbox}
  \begin{lstlisting}[language=bash]
# 自动识别网卡：有默认路由的为外网(WAN)，另一张为内网(LAN)
WAN_IF=$(ip route show default | awk '{print $5; exit}')
LAN_IF=$(ip -o link show | awk -F': ' -v w="$WAN_IF" '$2!="lo" && $2!=w{print $2; exit}')
...
\subsection{客户端配置（第二台 VM）}

NAT 实验需要两台虚拟机：一台做 NAT 网关（运行 \texttt{server\_setup.sh}），另一台做内网客户端。客户端只需一张 Host-Only 网卡（请在 VirtualBox 中禁用其自带的 NAT 网卡，避免绕过网关），并将默认网关指向 NAT 服务器的 Host-Only 地址。\texttt{client\_setup.sh} 默认保留客户端现有 IP，仅用 nmcli 设置默认网关与 DNS（如需更改 IP 才传第 2 个参数，且经 SSH 运行时会提示确认以避免断连）。脚本末尾自动进行端到端验证（默认路由、ping 网关、ping 8.8.8.8、curl 网页），输出与 \texttt{verify.sh} 相同的 \texttt{[PASS]/[FAIL]} 格式；其中“ping 网关”仅为参考（网关可能丢弃 ICMP），能否出网才是判据。
...
\begin{lstlisting}[language=bash]
# 服务器(网关)端：
sudo bash labs/lab8_NAT/server_setup.sh
sudo bash labs/lab8_NAT/verify.sh

# 客户端（第二台 VM，参数为服务器的 Host-Only IP）：
sudo bash labs/lab8_NAT/client_setup.sh 192.168.56.101
\end{lstlisting}
\end{cmdbox}
...
\begin{warnbox}
  例外：实验2（VPN）与实验8（NAT）为双机实验，安装脚本按角色拆分为
  \texttt{server\_setup.sh} 与 \texttt{client\_setup.sh}；实验2 的验收脚本相应为
  \texttt{server\_verify.sh} / \texttt{client\_verify.sh}（无单一 \texttt{verify.sh}）。
\end{warnbox}
