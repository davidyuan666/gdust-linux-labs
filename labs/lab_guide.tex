...
\begin{enumerate}[label=\arabic*.,leftmargin=*]
  ...
  \item 网络设置：选择\textbf{桥接网卡}，界面名称选宿主机正在使用的物理网卡
\end{enumerate}
...
为方便从 Windows 终端远程操作 Linux 虚拟机，推荐配置 SSH 服务和桥接模式网络。
...
\midrule
NAT        & 藏在宿主机NAT后 & 需端口转发 & ✓ \\
Host-Only  & 虚拟独立局域网  & ✓ 直接连 & ✗ \\
\textbf{桥接} & \textbf{接入物理局域网} & \textbf{✓ 直接连} & \textbf{✓} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{infobox}
  \textbf{推荐：桥接模式} \\
  桥接模式下虚拟机直接从路由器获取 IP（如 192.168.1.105），和宿主机在同一个局域网。\\
  Windows SSH 连接：\texttt{ssh 用户名@192.168.1.105}，无需任何额外配置。\\
  唯一注意：切换网络环境（如教室→宿舍）时 IP 会变化。
\end{infobox}

\subsubsection{配置步骤}

\begin{enumerate}[label=\arabic*.,leftmargin=*]
  \item VirtualBox → 设置 → 网络 → 网卡1 → 连接方式：\textbf{桥接网卡}
  \item 界面名称选择宿主机正在使用的物理网卡（Wi-Fi 或有线网卡）
  \item 启动虚拟机，查看 IP：
  \begin{cmdbox}
  \begin{lstlisting}[language=bash,numbers=none]
ip a | grep inet
